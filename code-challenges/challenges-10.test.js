...
const getNames = (arr) => {
  // Solution code here...
  const reversedNames = arr.map(element => {
...
const count = (target, input) => {
  ...
  return input.reduce((accumulator, value) => {
    value = value.reduce((accuTwo, valTwo) => {
      if (valTwo === target) {
        accuTwo += 1;
      }
      return accuTwo;
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((accu, val) => {
    val = val.reduce((accu, val) => {
      return accu += val;
    });
    return accu += val;
  }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(element => {
    return element.filter(num => {
      return !(num % 5) && (typeof num === 'number');
    }).map(num => {
...
let findShortest = (data) => {
  // Solution code here...
  const shortestNum = data.reduce((acc, val) => {
    let valHeight = parseInt(val.height);
    acc[0] = ( acc[0] === undefined || valHeight < acc[0] ) ? valHeight : acc[0];
    acc[1] = ( acc[1] === undefined || valHeight > acc[1] ) ? valHeight : acc[1];
    return acc;
  }, []);
  return data.filter(obj => {
    return (parseInt(obj.height) === shortestNum[0]);
  }).map(obj => {
    return obj.name;
  })[0];
};
